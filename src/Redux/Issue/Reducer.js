...
export const issueReducer = (state = initalState, action) => {
    switch (action.type) {
       ...
       case actionTypes.UPDATE_ISSUE_SUCCESS:
        return{
            ...state,loading:false,issueDetails:action.issues
        };
        ...
        return{
            ...state,loading:false,issues:state.issues.map(issue=>issue.id?action.issue:issue)
        };
        case actionTypes.DELETE_ISSUE_SUCCESS:
        return{
            ...state,loading:false,issues:state.issues.filter(issue=>issue.id!==action.issueId)
        };
